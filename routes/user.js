...
//---------------------------------------------add new member page call------------------------------------------------------
exports.new_member_add = function(req, res){
   message = '';
   if(req.method == "POST"){
      var post  = req.body;
...
exports.add_details = function(req, res){
   ...
   if(req.method == "POST"){
      ...
      var query = db.query(sql, function(err, result) {
         if(err){
            message = "Something went wrong!!";
            res.render('add_details.ejs',{message: message});
         }else{
...
exports.login = function(req, res){
   ...
   if(req.method == "POST"){
      ...
      db.query(sql, function(err, results){     
         if(results.length>0){
            req.session.userId = results[0].Login_ID;
            req.session.user = results[0];

            var dbpass = decrypt(results[0].Password);
